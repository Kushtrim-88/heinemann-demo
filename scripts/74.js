...
(function(global) {
    ...
    function retrieveProductRecommendations(id, sProductCode, sComponentId, sPosition, sTeaserId, sRequestContextProductCode) {
        var oElement = $("#" + id);
        ...
        $.get(ajaxUrl, {
            ...
            position: sPosition,
            teaserId: sTeaserId,
            requestContextProductCode: sRequestContextProductCode,
        }, addProductRecommendation(id)).done(function() {
            globalEmitter.emit('hydrate', oElement[0]);
    ...
    function loadProductRecommendations() {
        ...
        for (var i = 0; i < divs.length; i++) {
            if (divs[i].id.search("reco") > -1 && ($(divs[i]).data(sapProdRecoInitialized) !== true)) {
                ...
                var sPosition = $("#" + divs[i].id).data("position");
                var sTeaserId = $("#" + divs[i].id).data("teaserid");
                var sRequestContextProductCode = $("#" + divs[i].id).data("requestcontextproductcode");

                retrieveProductRecommendations(divs[i].id, sProductCode, sComponentId, sPosition, sTeaserId, sRequestContextProductCode);
